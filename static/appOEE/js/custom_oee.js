...
async function total_dias_horasParadas() {
    fetch('/oee/total_dias_horasParadas/') // O método padrão já é 'GET', não precisa declarar
        .then(response => {
...
async function carregarOeeDiarioParqueMaquinas() { // grafico OEE por maquina
    ...
    try {
        const resp = await (await fetch('/oee/oee_parque_diario/')).json();
...
async function carregarOeeDiarioParqueGlobal() { // calcula o OEE acumulado, disp., perf., qual. toda a planta
    ...
    try {
        const resp = await (await fetch('/oee/oee_parque_acumulado/')).json();
...
async function carregarOeePorMaquina(maquinaId) { //  OEE com escolha de maquina
    ...
    try {
        const resp = await (await fetch(`/oee/oee_por_maquina/?maquina=${maquinaId}`)).json();
        if (chartOEE) chartOEE.destroy();
...
async function carregarRankingOEE() { // ranking OEE
    ...
    try {
        const resp = await (await fetch('/oee/ranking_oee/')).json();
...
async function paradas_maquina() { // horas paradas acumuladas por maquina
    ...
    try {
        const resp = await (await fetch('/oee/paradas_maquina/')).json();
...
async function tipo_parada() {// tipos de paradas produtivas, improdutivas ou programadas
    ...
    try {
        const resp = await (await fetch('/oee/tipo_parada/')).json();
...
async function motivo_parada() { // Grafico motivos de paradas por maquina em horas
    ...
    try {
        const resp = await (await fetch('/oee/motivo_parada/')).json();
...
async function oee_maquina() { // Grafico  OEE  por maquina em %
    ...
    try {
        const resp = await (await fetch('/oee/oee_maquina/')).json();
...
async function prejuizos_financeiros() { // tabela prejuizos financeiros na dupont.html
    try {
        const resp = await (await fetch('/oee/prejuizos_financeiros/')).json();
...
async function prejuizos_custos_maquina() { // custo_minuto individual por máquina
    ...
    try {
        // Esta rota entrega o "preju_maq"
        const resp = await (await fetch('/oee/prejuizos_custos_maquina/')).json();
        console.log("Resposta de prejuizos_custos_maquina:", resp); // Log para verificar a resposta
        if (chartcustos_maquina) chartcustos_maquina.destroy();

        chartcustos_maquina = new Chart(canvas.getContext('2d'), {
            type: 'bar',
            data: { 
                labels: resp.maq_nome, 
                datasets: [{ label: 'Prejuízo (R$)', data: resp.custo_minuto }] // <-- Corrigido para ler preju_maq
            },
            options: configGlobal('', 'y', 'custo_minuto') // <-- Mudado para moeda, já que são os valores em R$
        });
...
async function prejuizos_maquina() { // prejuizo individual por maquina em R$
    const canvas = document.getElementById('Chartmaq_prejuizo');
    if (!canvas) return;

    try {
        // Esta rota entrega o "custo_minuto"
        const resp = await (await fetch('/oee/prejuizos_maquina/')).json();
        console.log("Resposta de prejuizos_maquina:", resp); // Log para verificar a resposta
        ...
        chartpreju_maquina = new Chart(canvas.getContext('2d'), {
            ...
            data: { 
                labels: resp.maq_nome, 
                datasets: [{ label: 'Custo por Minuto', data: resp.preju_maq }] // <-- Corrigido para ler custo_minuto
            },
            options: configGlobal('', 'y', 'moeda') // <-- Mudado para a configuração de custo_minuto
        });
...
async function carregarParadasDiarioParqueMaquinas() { // grafico Paradas por dia
    ...
    try {
        const resp = await (await fetch('/oee/paradas_parque_diario/')).json(); 
...
fetch('/oee/paradas_por_dia_maquina/')
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    carregarRankingOEE();
    // Altere a linha 750 para ficar assim:
    if (typeof oee_global_duponthtml === "function") {
