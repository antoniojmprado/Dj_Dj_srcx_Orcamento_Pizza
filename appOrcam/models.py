...
class MaquinaOEE(models.Model):
    ...
    Se 'custo_hora_operacional' não existe lá, ela NÃO deve estar aqui.
    """
...
class ParametrosOrcamento(models.Model):
...
class Orcamento(models.Model):
    ...
    chapa_ideal = models.ForeignKey(
        Chapa, on_delete=models.PROTECT, related_name='ideal_set')
    ...
    def save(self, *args, **kwargs):
        ...
        params = ParametrosOrcamento.objects.first()
        ...
        custo_material_unitario = self.chapa_utilizada.area_m2 * \
            (custo_papelao_m2 + custo_tinta_unitario)

        # 3. CUSTO DE MÁQUINAS (Cálculo OEE)
        custo_maquinas = Decimal('0.00')
        try:
            config = ConfiguracaoRateio.objects.get(
                maquina=self.maquina_impressao)
            velocidade = config.producao_un_hora if config.producao_un_hora > 0 else 1000
            tempo_unitario = Decimal(60) / Decimal(str(velocidade))
        ...
        custo_total = Decimal(str(custo_material_unitario)) + custo_maquinas
