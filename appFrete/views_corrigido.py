...
def calcular_frete_view(request):
    ...
    if request.method == 'POST':
        ...
        if 'comprimento' not in request.POST:
            ...
            print(f"Enviando para regiao.html com: {dic_prox_pag}")            
        ...
        for i in range(len(list_comps)):
            ...
            print(f"Calculando para item {i}: volume={v_i}")
            vol_total_m3 += v_i
            total_unidades += int(list_unis[i]) * int(list_vols[i])
        print(
            f"Volume total={vol_total_m3}, unidades={total_unidades}, kg informado={kg_total_informado}, valor NF={valor_total_nf}  | destino: {destino_cidade} - {destino_uf}")
        ...
        print("Tabela de transportadoras após cálculo de peso final:")
        print(f'Transportadora: {tab_transp["transportadora"].tolist()[0]}, Peso Cubado: {tab_transp["peso_cubado_calc"].tolist()[0]}, Peso Final: {tab_transp["peso_final"].tolist()[0]}')
        ...
        print(f'peso_final: {tab_transp["peso_final"].tolist()[0]}, peso_excesso: {tab_transp["peso_excesso"].tolist()[0]}, valor_excesso: {tab_transp["valor_excesso"].tolist()[0]}')
        
        print(f'ad_val_total: {tab_transp["ad_val_total"].tolist()[0]}, gris_total: {tab_transp["gris_total"].tolist()[0]}, pedagio_total: {tab_transp["pedagio_total"].tolist()[0]}')

        # Frete Peso (Calculado sobre os 100kg iniciais ou peso total dependendo da sua regra)
        tab_transp['frete_min'] = tab_transp['100_kg']
        print(f'frete_min: {tab_transp["frete_min"].tolist()[0]}')
        ...
        print(f'frete_net: {tab_transp["frete_net"].tolist()[0]}')
        print(f'frete_net: {tab_transp["frete_net"].tolist()[0]}')
        ...
        print(f'frete_final: {tab_transp["frete_final"].tolist()[0]}, frete_unidade: {tab_transp["frete_unidade"].tolist()[0]}')
        ...
        print(f"Resultados para o template: {list(lista_resultados)}")      
        ...
        print(f"Contexto final para template: {contexto}")
